...
General Use Font: ~CozetteVector~
...
#+END_SRC
*** Transparency
This sets transparency to 90%, so 90% opaque.
  ...
  (after! org-modern
    ...
    (setq org-modern-table nil
          ...
          '((t . t)
            ...
            ("export" . "󰈇"))))
#+END_SRC
*** Org Mode Image Download and Detailed Apperance Settings
...
Note: You DID switch to Wayland and this did break. So this is how you fixed it using grimshot:
#+BEGIN_SRC emacs-lisp :tangle config.el
...
#+END_SRC
***** Emphasis Markers
Markers like / for italics and * for bold are hidden by default, so this makes it hard to edit.
...
#+END_SRC
*** DONE Fix the spellchecker so it changes with the language, properly
...
*** SCP Settings
        - We can add these settings to prevent TRAMP from creating extra files and force it to use SCP directly when moving files.
          #+BEGIN_SRC emacs-lisp :tangle config.el
          ...
          #+END_SRC
*** Use Direct Async
- All emacs processes are either synchronous or asynchronous
- TRAMP used to create a new connection for every async process.
- Using direct async process makes this much faster.
  #+BEGIN_SRC emacs-lisp :tangle config.el
  ...
  #+END_SRC
*** LSP
- Yes, sadly you're going to have to disable LSP over TRAMP.
- We've tried it ourselves and it just isn't worth the slowdown.
  - Plus projectile is basically unusable with it.
  #+BEGIN_SRC emacs-lisp :tangle config.el
...
*** SQL
    ...
    - Tips to get this working:
      - Enable the port using ufw on your remote server hosting the mysql DB.
    ...
    - This is what the result should be if you check to make sure the user was created:
      - [[file:../../.config/doom/General_Doom_Settings/org_20260309-092529_screenshot.png]]
    ...
    #+end_src
*** SQL LSP
- If you don't set the correct =sql-product= then you'll get error messages for correct SQL code.
- By default sqls is expecting your DB info to be in a json that's in the root of your workspace.
  - This means in a project sql-test-code, you should have a file in the path of ~.sqls/config.json~ if you have it anywhere else it won't work with this config.
  - Also ensure that =sqls= is in your path to run this.
- Note: sqls is still under development so this setup might break in the future.
    #+BEGIN_SRC emacs-lisp :tangle config.el
      (after! lsp-mode
...
#+END_SRC
** Hotkeys
We save all the hotkeys here so there's in one place.
...
- Change spell-check dictionary: ~SPC t d~
...
This is the Linux desktop config.
...
#+end_src
** Completion
I use Corfu and ivy for in-buffer completion.
#+begin_src emacs-lisp :tangle no
       :completion
